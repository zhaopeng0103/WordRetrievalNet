...
xml_file = ["Konzilsprotokolle_Train_I_WL.xml", "Konzilsprotokolle_Train_II_WL.xml", "Konzilsprotokolle_Train_III_WL.xml"]


class Cfg:
    pre_path = "/home/zhaopeng/WordSpottingDatasets/"
    data_name = "Konzilsprotokolle"
    xml_path = os.path.join(pre_path, data_name, "train", "xml")
...
def load_dataset():
    ...
    for xml_name in xml_file:
        dom_tree = parse(os.path.join(Cfg.xml_path, xml_name))
        ...
        for spot in root_node.getElementsByTagName("spot"):
            ...
            file_label_dict[img_name].append({"x1": x1, "y1": y1, "x2": x2, "y2": y2, "word": word})
...
if __name__ == "__main__":
    ...
    inplace_augment(data, tparams=tparams)  # original data is kept here
