...
namespace lanms {
	 ...
	 */
	std::pair<std::vector<Polygon>, std::vector<size_t>> standard_nms(std::vector<Polygon> &polys, float iou_threshold) {
		size_t n = polys.size();
		if (n == 0)
			return {};
		...
		return std::make_pair(ret, keep);
	}

	std::pair<std::vector<Polygon>, std::vector<size_t>>
