...
namespace lanms_adaptor {
	...
	/**
	 * \param quad_n9 an n-by-9 numpy array, where first 8 numbers denote the quadrangle, and the last one is the score
	 * \param iou_threshold two quadrangles with iou score above this threshold will be merged
	 *
	 * \return an n-by-9 numpy array, the merged quadrangles
	 */
	std::pair<std::vector<std::vector<float>>, std::vector<size_t>> merge_quadrangle_n9(
			py::array_t<float, py::array::c_style | py::array::forcecast> quad_n9, float iou_threshold) {
		...
		auto res_pair = lanms::merge_quadrangle_n9(ptr, n, iou_threshold);
		return std::make_pair(polys2floats(res_pair.first), res_pair.second);
	}
